"""Grafo multi-agente (LangGraph): Supervisor -> especialistas (fan-out) -> síntese.
    ...
    clima  uso_terra       (cada um: create_agent ReAct com as suas tools)
       \\     /
...
from .specialists import build_clima_specialist, build_uso_terra_specialist
...
_NO_SPECIALIST_ANSWER = (
    ...
    "duas dimensões: clima e solo (Open-Meteo) e uso e cobertura da terra "
    "(MapBiomas). Reformule a pergunta nesses termos que eu consigo ajudar."
)
...
def _build_nodes(planner, clima_agent, uso_terra_agent, synthesizer):
    ...
    return supervisor, clima, uso_terra, synthesis
...
async def build_graph(
    ...
    *,
    ...
    if clima_agent is None or uso_terra_agent is None:
        if tools is None:
    ...
    supervisor, clima, uso_terra, synthesis = _build_nodes(
        planner, clima_agent, uso_terra_agent, synthesizer
    )
    ...
    builder.add_conditional_edges(
        "supervisor", _route_after_supervisor, ["clima", "uso_terra", END]
    )
