...
func newFakeAgentWithStatus(t *testing.T, status int) *fakeAgent {
	...
	mux.HandleFunc("/region/point", func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
	...
	mux.HandleFunc("/land_use/timeseries", func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		_, _ = w.Write([]byte(`{"available":true,"region_query":"` + r.URL.Query().Get("region") + `"}`))
	})
	mux.HandleFunc("/land_use/raster_overlay", func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		_, _ = w.Write([]byte(`{"available":true,"year":"` + r.URL.Query().Get("year") + `"}`))
	})
...
func TestRegionPointEhEncaminhadoProAgenteComQueryString(t *testing.T) {
	agent := newFakeAgent(t)
	defer agent.Close()

	mux := newMux(mustParseURL(t, agent.URL), NewRateLimiter(nil, 0, time.Minute), noCache(), noBreaker())
	req := httptest.NewRequest(http.MethodGet, "/api/region/point?query=Porto+Alegre", nil)
	rec := httptest.NewRecorder()
	mux.ServeHTTP(rec, req)
...
func TestLandUseTimeseriesEhEncaminhadoProAgenteComQueryString(t *testing.T) {
	agent := newFakeAgent(t)
	defer agent.Close()

	mux := newMux(mustParseURL(t, agent.URL), NewRateLimiter(nil, 0, time.Minute), noCache(), noBreaker())
	req := httptest.NewRequest(http.MethodGet, "/api/land_use/timeseries?region=RS&level=2", nil)
	rec := httptest.NewRecorder()
	mux.ServeHTTP(rec, req)
...
func TestLandUseRasterOverlayEhEncaminhadoProAgenteComQueryString(t *testing.T) {
	agent := newFakeAgent(t)
	defer agent.Close()

	mux := newMux(mustParseURL(t, agent.URL), NewRateLimiter(nil, 0, time.Minute), noCache(), noBreaker())
	req := httptest.NewRequest(http.MethodGet, "/api/land_use/raster_overlay?year=2025", nil)
	rec := httptest.NewRecorder()
	mux.ServeHTTP(rec, req)
	...
	if !strings.Contains(rec.Body.String(), `"year":"2025"`) {
		t.Fatalf("query string não repassada como esperado: %s", rec.Body.String())
	}
}
